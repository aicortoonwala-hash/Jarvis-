...
class CommandRouter(private val context: Context, private val memory: MemoryStore) {
    fun execute(command: String): String {
        ...
        return when {
            ...
            isPlayRequest(c) -> playRequest(original)
    ...
    private fun youtubeSearch(original: String): String {
        ...
        val query = when {
            ...
            .trim()
        if (query.isBlank()) { open("https://www.youtube.com"); return "Opening YouTube." }
        open("https://www.youtube.com/results?search_query=" + Uri.encode(query))
        return "Searching YouTube for $query."
    }

    private fun isPlayRequest(c: String): Boolean =
        (c.contains("lagao") || c.contains("chalao") || c.contains("play ") || c.contains("bajao")) &&
            !c.contains("open settings")

    private fun playRequest(original: String): String {
        val lower = original.lowercase()
        var query = original
        listOf("lagao", "chalao", "play", "bajao", "please", "karo", "kar do", "karna").forEach { word ->
            query = query.replace(Regex("(?i)\\b${Regex.escape(word)}\\b"), " ")
        }
        query = query.replace(Regex("(?i)\\b(youtube|par|pe)\\b"), " ")
    ...
    private fun openCamera(): String = try {
        context.startActivity(Intent(MediaStore.ACTION_IMAGE_CAPTURE).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)); "Opening camera."
    ...
    private fun openMaps(original: String): String {
        ...
        return try { context.startActivity(Intent(Intent.ACTION_VIEW, uri).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)); "Opening maps${if (query.isNotBlank()) " for $query" else ""}." }
    ...
    private fun callContactOrNumber(nameOrNumber: String): String {
        ...
        val number = findContactNumber(nameOrNumber); val target = number ?: nameOrNumber
        return try {
            if (number != null && ContextCompat.checkSelfPermission(context, Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED) {
                context.startActivity(Intent(Intent.ACTION_CALL, Uri.parse("tel:" + Uri.encode(target))).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)); "Calling $nameOrNumber."
            } else { context.startActivity(Intent(Intent.ACTION_DIAL, Uri.parse("tel:" + Uri.encode(target))).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)); "Opening the dialer for $nameOrNumber." }
        } catch (_: Exception) { "I couldn't start the call." }
    }

    private fun findContactNumber(name: String): String? = try {
        val projection = arrayOf(ContactsContract.CommonDataKinds.Phone.NUMBER)
        val selection = "${ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME} LIKE ?"
        val args = arrayOf("%$name%")
        context.contentResolver.query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI, projection, selection, args, null)?.use { cursor -> if (cursor.moveToFirst()) cursor.getString(0) else null }
    } catch (_: Exception) { null }

    private fun composeSms(original: String): String {
        val text = original.substringAfter(" ").trim(); if (text.isBlank()) return "Tell me the phone number and message."
        context.startActivity(Intent(Intent.ACTION_SENDTO, Uri.parse("smsto:")).apply { putExtra("sms_body", text); addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }); return "Opening SMS composer."
    }

    private fun openWhatsApp(): String = try {
        context.startActivity(context.packageManager.getLaunchIntentForPackage("com.whatsapp")?.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) ?: Intent(Intent.ACTION_VIEW, Uri.parse("https://wa.me/")).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)); "Opening WhatsApp."
    } catch (_: Exception) { "WhatsApp is not available." }

    private fun rememberCommand(text: String): String {
        val parts = text.split(" is ", limit = 2); if (parts.size != 2 || parts[0].isBlank() || parts[1].isBlank()) return "Tell me what to remember, for example: remember name is Alex."
        memory.remember(parts[0], parts[1]); return "Got it. I will remember ${parts[0].trim()}."
    }
    ...
    private fun recallCommand(original: String): String {
        ...
        if (key.isBlank() || key == "what do you remember") { val all = memory.all(); return if (all.isEmpty()) "I don't have any saved memories yet." else "I remember: " + all.entries.joinToString(", ") { "${it.key} is ${it.value}" } + "." }
